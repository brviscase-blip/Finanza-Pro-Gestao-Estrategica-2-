...
const Mensal: React.FC<MensalProps> = ({ entries, setEntries, financialData, isHeaderPinned, activeYear }) => {
  ...
  const getStatusBadge = (status: PaymentStatus) => {
    ...
    switch (status) {
      ...
      case 'Pendente': return <span className={`${base} bg-white dark:bg-[#0A1022] text-slate-400 border-slate-200 dark:border-slate-800`}><Clock className="absolute left-2 top-1/2 -translate-y-1/2 w-3.5 h-3.5" /> PENDENTE</span>;
    ...
    <div className="w-full relative">
      ...
      >
        <div className={`transition-all duration-500 ${!isHeaderVisible ? '-translate-y-full opacity-0 pointer-events-none' : 'translate-y-0 opacity-100 pointer-events-auto'}`}>
          <div className="bg-white dark:bg-[#0A1022] border-2 border-slate-200 dark:border-slate-800 rounded-xl shadow-lg p-4 flex flex-col gap-4 relative">
      ...
      <div className="space-y-6 pb-20">
        {filteredSortedMonths.length === 0 ? (
          ...
          filteredSortedMonths.map(month => {
            ...
            return (
              <div key={month} id={`month-section-${month}`} className="scroll-mt-[250px] min-w-[1400px] bg-white dark:bg-[#0A1022] rounded-xl border-2 border-slate-200 dark:border-slate-800 shadow-sm relative overflow-visible">
                ...
                {!isCollapsed && (
                  <div className="animate-in slide-in-from-top-2 duration-300 relative px-2 pb-2 overflow-visible">
                    <div className={`grid ${gridTemplate} text-[10px] font-black uppercase tracking-widest text-white bg-[#0F172A] dark:bg-[#0A1022] shadow-md h-[34px] items-center rounded-t-lg`}>
                        ...
                        monthEntries.map((entry: MonthlyEntry) => {
                          ...
                          return (
                            <div key={entry.id} className={`grid ${gridTemplate} min-h-[40px] items-center cursor-pointer relative transition-all duration-100 rounded-md ${isPaid ? 'bg-emerald-500/10 dark:bg-emerald-500/5 z-[5]' : 'hover:bg-slate-200/40 dark:hover:bg-[#0F172A]/20'}`}>
                              ...
                              <div className="px-1 py-1 flex items-center justify-center min-w-0 h-full"><div className="relative flex items-center justify-center h-[27px] px-1 bg-white dark:bg-[#0A1022] border-2 border-slate-200 dark:border-slate-800 rounded-md shadow-sm w-full"><DollarSign className="absolute left-1 top-1/2 -translate-y-1/2 w-3 h-3 text-slate-400 dark:text-slate-600" /><div className="flex items-center ml-2"><span className="text-[11px] font-black text-slate-400 dark:text-slate-600 mr-0.5">R$</span><span className="text-[11px] font-black text-slate-900 dark:text-slate-100 tabular-nums">{entry.estimatedValue.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span></div></div></div>
                              <div className="px-1 py-1 flex items-center min-w-0 h-full justify-center"><div className="relative flex items-center gap-1.5 px-1 h-[27px] bg-white dark:bg-[#0A1022] border-2 border-slate-200 dark:border-slate-800 rounded-md shadow-sm w-full justify-center"><CalendarDays className="absolute left-1 top-1/2 -translate-y-1/2 w-3 h-3 text-slate-400" /><span className="text-[11px] font-black text-slate-900 dark:text-white tabular-nums ml-2">{formatDate(entry.dueDate)}</span></div></div>
